...
def tranform_tree(tree: dict, rules: dict):
    ...
    # print("ATTRS:", v["attrs"])
    new_tree[k] = {
        "attrs": v["attrs"] if not isinstance(v["attrs"], str) else Markup(Template.handle_text_nodes(v["attrs"], re.compile(r"\?VAR:&lt;(.*?)&gt;\?"))).unescape(),
...
class Template:
    ...
    def __init_subclass__(cls):
        ...
        cls._name = kebab_case_name(cls.__name__)
        cls._id = cls._name + "_hypie_template"
    ...
    def register_template(cls):
        ...
        return htpy.script(type="text/hyperscript-template", id=cls._id)[
            transformed_htpy
        ]
...
class ClientFragment:
    ...
    def __init_subclass__(cls):
        ...
        for f in dataclasses.fields(cls):
            setattr(cls, f.name, TemplatedVariableLiteral(VariableLiteral(f.name)))
        setattr(cls, "field_names", [f.name for f in dataclasses.fields(cls)])
        cls._fragment_name = kebab_case_name(cls.__name__)
        cls._template_id = cls._fragment_name + "-hypie-template"
    ...
    def __html__(self):
        el = self.template()
        new_el = tree_to_htpy(transform_root(htpy_to_tree(el), self._fragment_name))
        return new_el
