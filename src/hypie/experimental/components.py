...
@dataclass_transform()
class ServerFragment(metaclass=ServerFragmentMeta):
    children = None
    ...
    def __init_subclass__(cls):
        # cls._hs_dsl_component = True
        cls._hypie_server_fragment = True
        # cls.component_name = kebab_case_name(cls.__name__)
        cls._fragment_name = kebab_case_name(cls.__name__)
        cls._tag = f"div[data-hypie-component='{cls._fragment_name}']"
    ...
    def generate_style(cls, with_style_tags=True):
        output_style = cls.style()
        if output_style:
            css = generate_scoped_css(cls._tag, cls.style())
            if with_style_tags:
                return htpy.style[Markup(css)]
            return Markup(css)
