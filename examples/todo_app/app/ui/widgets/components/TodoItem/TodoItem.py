...
class TodoItem(hp.ServerComponent):
    id: int
    title: str
    done: bool
    ...
    def template(self):
        return htpy.li(id=f"todo-{self.id}", class_="flex gap-2")[
            htpy.a(
                ...
                _=hp.On("click")[hp.trigger(ToggleTodoEvent(id=self.id))],
            )["Toggle"],
            htpy.a(
                class_="text-red-500 cursor-pointer select-none",
                _=hp.On("click")[
                    hp.trigger(RequestRemoveTodoEvent(id=self.id, title=self.title))
                ],
            )["Delete"],
...
class TodoDeleteModal(hp.ClientComponent):
    todo_id: int
    title: int
